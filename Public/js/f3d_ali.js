...
//注册交互点击
function initClick(tdtMap, scene, render, camera, demMesh, bboxGeo,
    lineDashMaterial, minEle, maxEle) {
    ...
    const fontSize = calFontSizeByGeoBBox(bboxGeo)
    const mesh_width_seg = demMesh.geometry.parameters.widthSegments
    const ring_radius = 0.25 / mesh_width_seg
    const dot_geometry = new THREE.CircleGeometry(ring_radius / 20, 32);
    const dot_meterial = new THREE.MeshBasicMaterial({
        color: '#ff0000',
    ...
    $('#threeCon1').on('mouseenter', function () {
        ...
        $(this).on('mouseup', function (mouseUpEvent) {
            ...
            if (timeInterval < 200) {
                ThreeClickHD(mouseUpEvent, tdtMap, render, camera, demMesh, fontSize)
            }
            mouseDownTime = 0
        })
    })
    ...
    //3D窗口点击事件
    function ThreeClickHD(event, tdtMap, render, camera, mesh, fontSize) {
        ...
        const mesh_col = mesh.geometry.parameters.widthSegments
        const mesh_row = mesh.geometry.parameters.heightSegments
        const spere_radius = 150 / Math.min(mesh_col / geo_width, mesh_row / geo_height)
        ...
        if (intersects.length > 0) {
            ...
            //相交片元的法线
            const face_normal = intersects[0].face.normal.clone()
            const worldNormal = face_normal.applyMatrix3(new THREE.Matrix3().getNormalMatrix(mesh.matrixWorld));
            ...
            p_x_geo = (minX + (p_x * geo_width) / dem_width)
            p_y_geo = (minY + (p_y * geo_height) / dem_height)

            //计算投影坐标
            const p_x_geo_proj = LatLon2XY(p_x_geo,p_y_geo).x
            const p_y_geo_proj = LatLon2XY(p_x_geo,p_y_geo).y
            dmd_list.push([p_x_geo_proj,p_y_geo_proj])
            const sp_x = dmd_list[0][0]
            const sp_y = dmd_list[0][1]
            const dis_p = Math.sqrt(Math.pow(p_x_geo_proj -sp_x,2) +Math.pow(p_y_geo_proj -sp_y,2)).toFixed(2)
            // 🎯 在这里添加点击数据到表格
            const clickData = {
                x: parseFloat(p_x_geo_proj.toFixed(4)),
                y: parseFloat(p_y_geo_proj.toFixed(4)),
                lng: parseFloat(p_x_geo.toFixed(5)),
                lat: parseFloat(p_y_geo.toFixed(5)),
                ele: ele,
                dis:dis_p,
            };

            // 调用addClickData函数将数据添加到表格
            if (typeof addClickData === 'function') {
                addClickData(clickData);
            } else {
                console.warn('addClickData函数未定义');
            }

            //three中
            const label_line_geometry = new THREE.BufferGeometry().setFromPoints([point, ep])
            setupLineProgress(label_line_geometry, [point, ep])

            const dem_marker = new THREE.Line(label_line_geometry, lineDashMaterial)
            dem_marker.renderOrder = 999;
            dem_marker.frustumCulled = false;
            threeClickGroup.add(dem_marker)

            const dem_click_info = createCoordSprite(click_point_index, dis_p, ele, ep, '#ffffff', fontSize)
            threeClickGroup.add(dem_click_info)

            //点击-11.14
            const ring_geometry = new THREE.CircleGeometry(ring_radius, 4);
            const ring_meterial = new THREE.MeshBasicMaterial({
                color: '#00ffa1',
                side: THREE.DoubleSide,
            })
            const dem_click_ring = new THREE.Mesh(ring_geometry, ring_meterial)
            dem_click_ring.position.copy(point.clone().setY(point.y + 0.00001))
            dem_click_ring.lookAt(dem_click_ring.position.clone().add(worldNormal));
            dem_click_ring.rotateZ(-Math.PI / 2);
            threeClickGroup.add(dem_click_ring)

            dem_click_ring.updateWorldMatrix(true, false);
            const positionAttribute = dem_click_ring.geometry.getAttribute('position');
            const count = positionAttribute.count;
            const vector = new THREE.Vector3();
            for (let i = 0; i < count; i++) {
                vector.fromBufferAttribute(positionAttribute, i);
                dem_click_ring.localToWorld(vector)
                const dot = new THREE.Mesh(dot_geometry, dot_meterial)
                dot.position.copy(vector.clone().setY(vector.y + 0.00001))
                dots.push(dot)
                threeClickGroup.add(dot)
            }
            ...
            const geo_marker = new T.Marker(new T.LngLat(p_x_geo, p_y_geo), { icon: icon });
...
function createCoordSprite(clickIndex, dis, ele, point, text_color, fontSize) {
    const scale = fontSize <= 8 ? 8 : 4;
    // 创建高分辨率Canvas
    const canvas = document.createElement('canvas');
    const context = canvas.getContext('2d', { willReadFrequently: true });
...
async function c_point_photo(scene, tdtMap, meshDem, bboxDem, photoPointList, bboxGeo, lineDashMaterial) {
    const raycaster = new THREE.Raycaster()
    const fontSize = calFontSizeByGeoBBox(bboxGeo)
    const loader = new THREE.FontLoader()
    ...
    for (let i = 0; i < inBboxPointList.length; i++) {
        ...
        if (intersects.length > 0) {
            ...
            const label_line_geometry = new THREE.BufferGeometry().setFromPoints([point, ep])
            setupLineProgress(label_line_geometry, [point, ep])
            const label_line = new THREE.Line(label_line_geometry, lineDashMaterial)
            label_line.renderOrder = 999;
            label_line.frustumCulled = false;
            bridgeGroup.add(label_line)

            const reader = new FileReader();
            reader.onload = function (e) {
                const textureLoader = new THREE.TextureLoader();
                textureLoader.load(e.target.result, (texture) => {
                    createSpriteFromTexture(texture, ep, bridgeGroup);
                });
            };
...
function createSpriteFromTexture(texture, cross_point, feature_group) {
    const material = new THREE.SpriteMaterial({
        map: texture,
        transparent: true,
        opacity: 0.9
...
function setupPhotoClickDetection(renderer, camera, scene) {
    ...
    function onMouseClick(event) {
        ...
        if (photoSprite) {
            console.log('点击了照片:', photoSprite.object.userData.fileName);
            console.log('坐标:', photoSprite.object.userData.coordinates);
            console.log('交点位置:', photoSprite.point);

            // 触发照片点击事件
        } else {
            console.log('点击了其他对象或空白区域');
        }
    }
