...
function createPointsLine(start, end, pointCount = 100) {
    const points = [];
    ...
    const material = new THREE.PointsMaterial({
        color: 0xff0000,
        size: 0.1,          // 点的大小
        sizeAttenuation: false, // 禁用衰减，保持固定大小
        depthTest: false,
        depthWrite: false
    });

    const pointsLine = new THREE.Points(geometry, material);
    pointsLine.renderOrder = 1000;
    pointsLine.frustumCulled = false;
...
//生成mesh_compass
async function c_mesh_compass(box3, mesh_name) {
    return new Promise((resolve, reject) => {
        ...
        loader.load(
            ...
            function (font) {
                ...
                const text_pos = new THREE.Vector3(0, box3.max.y, box3.min.z)
...
//生成bridge_point_list
async function c_point_bridge(scene, meshDem, box3, bridgePointList, bboxGeo) {
...
//加载纹理
async function loadTexture(mesh, tiles) {
    const textureLoader = new THREE.TextureLoader()
    textureLoader.load(tiles.toDataURL('image/png'), function (tiles) {
        mesh.material.map = tiles
...
//注册交互点击
async function initClickRing(drawRect, tdtMap, scene, render, camera, demMesh, box3, bboxGeo, markerList) {
    deleteGroup(scene, 'three_click_group')
    const threeClickGroup = new THREE.Group()
    threeClickGroup.name = 'three_click_group'

    const ray_caster_mesh_click = new THREE.Raycaster()
    const mouse = new THREE.Vector2()

    const geo_bounds_min_x = bboxGeo.getSouthWest().lng
    const geo_bounds_min_y = bboxGeo.getSouthWest().lat
    const geo_bounds_max_x = bboxGeo.getNorthEast().lng
    const geo_bounds_max_y = bboxGeo.getNorthEast().lat
    const geowidth = geo_bounds_max_x - geo_bounds_min_x
    const geoheight = geo_bounds_max_y - geo_bounds_min_y
    const demwidth = box3.max.x - box3.min.x
    const demheight = box3.max.z - box3.min.z
    const mesh_col = demMesh.geometry.parameters.widthSegments
    const mesh_row = demMesh.geometry.parameters.heightSegments
    const spere_radius = 150 / Math.min(mesh_col / geowidth, mesh_row / geoheight)
    ...
    const ring_mesh = new THREE.Mesh(ring_geometry, ring_wave_material);
    ring_mesh.rotateX(-Math.PI / 2);
    ring_mesh.position.set(0, 0.0001, 0)
    ring_mesh.userData.material = ring_wave_material;
    ...
    const geo_marker = new T.Marker(new T.LngLat(0, 0), { icon: icon });
    ...
    //3D窗口点击事件
    function ThreeClickHD(event, tdtMap, render, camera, mesh, markerList) {
        ...
        if (intersects.length > 0) {
            //three
            const point = intersects[0].point
            threeClickGroup.add(label_line_mesh)
            label_line_mesh.position.copy(point)
            threeClickGroup.add(ring_mesh)
            ring_mesh.userData.material.uniforms.circleCenter.value = new THREE.Vector3(point.x, 0, point.z)
            scene.add(threeClickGroup)

            //tdt
            const canvas_x = intersects[0].point.x
            const canvas_z = intersects[0].point.z
            const p_x = canvas_x + demwidth / 2
            const p_y = demheight / 2 - canvas_z
            const p_x_geo = (geo_bounds_min_x + (p_x * geowidth) / demwidth).toFixed(5)
            const p_y_geo = (geo_bounds_min_y + (p_y * geoheight) / demheight).toFixed(5)
            tdtMap.addOverLay(geo_marker)

            geo_marker.setLngLat(new T.LngLat(p_x_geo, p_y_geo))
        }
    }
    ...
    drawRect.addEventListener('mousedown', e =>
        TDTClickHD(e, tdtMap, demMesh)
    )
    async function TDTClickHD(e, tdtMap, meshDem) {
        //tdt
        ...
        if (!geo_marker) {
            ...
            geo_marker = new T.Marker(new T.LngLat(jd, wd), { icon: icon });
            tdtMap.addOverLay(geo_marker)
            geo_marker_list.push(geo_marker)
        } else {
            geo_marker.setLngLat(new T.LngLat(jd, wd))
        }

        //three
        const three_pos_x = ((jd - geo_bounds_min_x) * demwidth) / geowidth     //将选择框缩放至dem大小，p点位置距dem左上角x距离等比缩放
        const three_pos_x_trans = three_pos_x - demwidth / 2         //左上坐标系平移至中间点坐标系
        const three_pos_z = ((wd - geo_bounds_min_y) * demheight) / geoheight
        const three_pos_z_trans = demheight / 2 - three_pos_z        //左下坐标系平移至中间点坐标系
        const startY = meshDem.geometry.boundingBox.max.y + 10
        const direction = new THREE.Vector3(0, -1, 0)
        const rayOrigin = new THREE.Vector3(
            three_pos_x_trans,
    ...
    return [ring_mesh, label_line_mesh]
}
...
async function c_polyline_riv_animate(scene, meshDem, box3, riverInsList, bboxGeo, material) {
    ...
    for (let i = 0; i < riverInsList.length; i++) {
        ...
        const fontSize = calFontSizeByGeoBBox(bboxGeo)
        const riv_label = cal_mid_p_riv_label(riverInsList[i][1], river_point_list_sorted, spere_radius, fontSize)
...
//计算河流的里程中点，用于注记河流名称
function cal_mid_p_riv_label(rivName, rivPointList, sphereRadius, fontSize) {
    ...
    for (let i = 1; i < rivPointList.length; i++) {
        ...
        if (p_m > riv_three_len / 2) {
            const riv_label = createTextSprite(rivName, rivPointList[i], sphereRadius, '#366ee5', fontSize)
            return riv_label
        }
    }
}

function createTextSprite(text, point, spere_radius, text_color, fontSize) {
    const scale = fontSize <= 8 ? 8 : 4
    // 创建高分辨率Canvas
    const canvas = document.createElement('canvas')
    const context = canvas.getContext('2d', { willReadFrequently: true })
    ...
    sprite.position.y += spere_radius * 4; // 在球体上方偏移
...
function regSHThreeVec(showFlag, domID, imgMaterial, vecMaterial, demMesh) {
    ...
    // 移除旧的处理器
    if (clickHandlers[domID]) {
        bt.removeEventListener('click', clickHandlers[domID]);
    }

    // 创建新处理器
    clickHandlers[domID] = function () {
        bt.classList.toggle('switch_active');
        let circle = $('#' + domID).find('.switch_circle')[0];
        circle.classList.toggle('circle_right');
    ...
    // 注册新处理器
    bt.addEventListener('click', clickHandlers[domID]);
}
...
async function c_point_photo(scene, tdtMap, meshDem, box3,
    photoPointList, bboxGeo, markerList, eleList, minEle, maxEle) {
    const raycaster = new THREE.Raycaster()
    ...
    for (let i = 0; i < inBboxPointList.length; i++) {
        ...
        if (intersects.length > 0) {
            ...
            //计算爬升高度信息
            const photo_ele = (point.y * (maxEle - minEle) + minEle)
            eleList.push(photo_ele)
            ...
            reader.onload = function (e) {
                ...
                textureLoader.load(e.target.result, (texture) => {
                    ...
                    createSpriteFromTexture(texture, ep, inBboxPointList[i][2], photoGroup, aspectRatio, photo_ele, i + 1);
                });
            };
...
function createSpriteFromTexture(texture, cross_point, photo_name, feature_group, aspect_ratio,photoEle, number) {
    const material = new THREE.SpriteMaterial({
        map: texture,
        transparent: true,
        opacity: 0.9
    ...
    const textureNumber = new THREE.CanvasTexture(canvas);
    const materialNumber = new THREE.SpriteMaterial({ map: textureNumber, transparent: true });
    const spriteNumber = new THREE.Sprite(materialNumber);
    spriteNumber.scale.set(0.05, 0.05, 1);
    spriteNumber.position.copy(cross_point).add(new THREE.Vector3(0, 0.05, 0)); // 稍微上移
    feature_group.add(spriteNumber);
}

function setupPhotoClickDetection(renderer, camera, scene,eleList) {
    const raycaster = new THREE.Raycaster();
    const mouse = new THREE.Vector2();
    ...
    function onMouseClick(event) {
        ...
        if (photoGroup) {
            ...
            if (photoSprite) {
                const ele = photoSprite.object.userData.ele.toFixed(2)
                $('#ele').text('当前海拔:' + ele + '米')
                const min_ele = Math.min(...eleList).toFixed(2)
                const max_ele = Math.max(...eleList).toFixed(2)
                $('#ele_range').text('最低海拔:' + min_ele + '最高海拔' + max_ele)
                const progress = (((ele - min_ele) / (max_ele - min_ele))*100.0).toFixed(2)
                $('#ele_progress').text('登顶进度:' + progress + '%')
    ...
    $(renderer.domElement).on('click', onMouseClick);
...
function highlightPhoto(sprite) {
    ...
    if (texture && texture.image) {
        ...
        // 创建新窗口
        photoPreviewWindow = window.open('about:blank', 'photoPreview', 'left=1100,top=200,width=600,height=900');
...
function setButtons() {
    ...
    if ($('.sw_container_false').find('button')[0].classList == 'switch switch_active') {
        $('.sw_container_false').find('button')[0].classList.toggle('switch_active')
        $('.sw_container_false').find('.switch_circle')[0].classList.toggle('circle_right')
    }

    //设置显示flag
    for (let key in showFlag) {
        if (showFlag[key] != 'sh_vec_tile') {
            if (showFlag[key] == false) {
                showFlag[key] = true
            }
        } else {
            if (showFlag[key] == true) {
                showFlag[key] = false
            }
        }
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const promise = new Promise((resolve) => {
            EXIF.getData(file, function () {
                ...
                // 提取拍摄时间
                const dateTime = allExifData.DateTime || allExifData.DateTimeOriginal || null;

                resolve([jd, wd, file.name, file, dateTime]);
            });
        });
