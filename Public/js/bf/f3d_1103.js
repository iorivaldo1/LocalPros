...
//生成bridge_point_list
async function c_point_bridge(demData, bbox_dem, pointList, bbox_geo, min_ele, max_ele) {
    const pointGeoList = []
    //读取dem数据
    ...
    //标志点大小
    const spere_radius = 0.4 * Math.min(width / geo_width, height / geo_height)
    inBboxPointList.forEach(p => {
        ...
        const three_pos_y_trnas = (ele - min_ele) / (max_ele - min_ele) //归一后的Y值
        ...
        sphere.position.copy(
            new THREE.Vector3(three_pos_x_trans, three_pos_y_trnas, three_pos_z_trans)
        )
...
async function c_polyline_river(demData, bboxDem, riverInsList, bboxGeo) {
    ...
    for (let i = 0; i < riverInsList.length; i++) {
        ...
        for (let j = 0; j < riverInsList[i][0].length; j++) {
            ...
            const three_pos_y_trnas =
                (ele - minElevation) / (maxElevation - minElevation) //归一后的Y值
            const three_pos_x = ((geoPoint[0] - dragBoxMinX) * dem_width) / geo_width //将选择框缩放至dem大小，p点位置距dem左上角x距离等比缩放
            const three_pos_z =
                ((geoPoint[1] - dragBoxMinY) * dem_height) / geo_height //将选择框缩放至dem大小，p点位置距dem左上角y距离等比缩放
            const three_pos_x_trans = three_pos_x - dem_width / 2 //左上坐标系平移至中间点坐标系
            const three_pos_z_trans = dem_height / 2 - three_pos_z //左上坐标系平移至中间点坐标系
...
//创建河流点--取消全局minele,maxele
async function c_point_river_fixed(
    demData,
    bboxDem,
    riverInsList,
    bboxGeo,
    ...
    // const canvasBbox = image.getBoundingBox();
    ...
    const geoX = geoBoundsMinX
    const geoY = geoBoundsMaxY

    //计算x,y方向的像元大小
    // const pixelSizeX = (canvasBbox[2] - canvasBbox[0]) / canvasWidth; //每一个像素占多少经度
    // const pixelSizeY = (canvasBbox[3] - canvasBbox[1]) / canvasHeight;//每一个像素占多少纬度
    const demWidth = bboxDem.max.x - bboxDem.min.x
    const demHeight = bboxDem.max.z - bboxDem.min.z

    //原始dem(没有放在three中进行拉伸)
    ...
    for (let i = 0; i < riverInsList.length; i++) {
        for (let j = 0; j < riverInsList[i][0].length; j++) {
            ...
            //高程
            const col = Math.floor((geoPoint[0] - geoX) / resolution) //从左上角开始，第几列
            const row = Math.floor((geoY - geoPoint[1]) / resolution) //从左上角开始，第几行
            const cnIndex = row * canvasWidth + col //内存位置
            const ele = rasters[0][cnIndex]
            const three_pos_y_trnas = (ele - minEle) / (maxEle - minEle) //归一后的Y值

            //平面
            const three_pos_x = ((geoPoint[0] - geoBoundsMinX) * demWidth) / geoWidth //将选择框缩放至dem大小，p点位置距dem左上角x距离等比缩放
            const three_pos_z =
                ((geoPoint[1] - geoBoundsMinY) * demHeight) / geoHeight //将选择框缩放至dem大小，p点位置距dem左上角y距离等比缩放
            const three_pos_x_trans = three_pos_x - demWidth / 2 //左上坐标系平移至中间点坐标系
            const three_pos_z_trans = demHeight / 2 - three_pos_z //左上坐标系平移至中间点坐标系
...
//画tdt中的dem网格线
async function drawGridAndEleLabel(DEMUrl, bounds, demOverLayerList, showLableFlag = 0) {
...
async function drawGridAndEleLabelYS(DEMUrl, bounds, demOverLayerList) {
    const demData = await fetch(DEMUrl)
    const arrayBuffer = await demData.arrayBuffer()
    const tiff = await GeoTIFF.fromArrayBuffer(arrayBuffer)
    const image = await tiff.getImage()
...
//将计算好的河流点，写入scene
async function c_p_r_ins_mesh(meshDem, bboxDem, riverInsList, bboxGeo) {
    const geoBoundsMinX = bboxGeo.getSouthWest().getLng()
    const geoBoundsMinY = bboxGeo.getSouthWest().getLat()
    const geoBoundsMaxX = bboxGeo.getNorthEast().getLng()
    const geoBoundsMaxY = bboxGeo.getNorthEast().getLat()
    ...
    for (let i = 0; i < riverInsList.length; i++) {
        for (let j = 0; j < riverInsList[i][0].length; j++) {
            ...
            if (three_pos_x_trans == bboxDem.min.x) {
                three_pos_x_trans += 0.00000001
            } else if (three_pos_x_trans == bboxDem.max.x) {
                three_pos_x_trans -= 0.00000001
            } else if (three_pos_z_trans == bboxDem.min.z) {
                three_pos_z_trans += 0.00000001
            } else if (three_pos_z_trans == bboxDem.max.z) {
                three_pos_z_trans -= 0.00000001
            }
            //高程
            const startY = meshDem.geometry.boundingBox.max.y + 10
            ...
            if (intersects.length > 0) {
                ...
                //有没有交叉到的点
                console.log(bboxDem, three_pos_x_trans, three_pos_z_trans)
            }
        }
    }
...
//生成与tdt中dem网格线相交的所有河流点
async function cal_riv_cross_dem_grid(rivIns, gridLines) {
    const result = []

    for (let i = 0; i < rivIns.length; i++) {
        const originalPoints = rivIns[i][0]
        const newPoints = []

        for (let j = 0; j < originalPoints.length; j++) {
            // 添加当前点
            newPoints.push(originalPoints[j])

            // 如果不是最后一个点，检查与网格线的交点
            if (j < originalPoints.length - 1) {
                const riv_line_qd = [originalPoints[j].lng, originalPoints[j].lat]
                const riv_line_zd = [
                    originalPoints[j + 1].lng,
                    originalPoints[j + 1].lat
                ]
                const riv_line = [riv_line_qd, riv_line_zd]

                const intersections = []

                for (let k = 0; k < gridLines.length; k++) {
                    const ins_point1 = linesIntersectWithPoint(gridLines[k], riv_line)
                    if (ins_point1.intersects) {
                        const cross_point = new T.LngLat(
                            ins_point1.intersectionPoint[0],
                            ins_point1.intersectionPoint[1]
                        )
                        intersections.push(cross_point)

                        // var circle = new T.Circle(cross_point, 2);
                        // map.addOverLay(circle)
                        // geo_marker_list.push(circle)
                    }
                }
        ...
        result.push([newPoints, rivIns[i][1]])
    }
...
//生成与mesh中网格线相交的所有河流点
async function cal_riv_cross_mesh_grid(scene, meshDem, bboxDem, riverInsList, bboxGeo) {
    const raycaster = new THREE.Raycaster()

    deleteGroup(scene, 'tp')
    ...
    for (let i = 0; i < riverInsList.length; i++) {
        ...
        for (let j = 0; j < riverInsList[i][0].length; j++) {
            ...
            if (intersects.length > 0) {
                ...
                const sphere = new THREE.Mesh(geometry, material)
                sphere.position.copy(point)
                resultGroup.add(sphere)
            } else {
                //有没有交叉到的点
                console.log(bboxDem, three_pos_x_trans, three_pos_z_trans)
            }
            riv_point_three.push([three_pos_x_trans, three_pos_z_trans])
        }
        ...
        for (let j = 0; j < riv_point_three.length - 1; j++) {
            const riv_sp = riv_point_three[j]
            const riv_ep = riv_point_three[j + 1]
...
async function c_polyline_riv(scene, meshDem, bboxDem, riverInsList, bboxGeo) {
    ...
    //标志点大小
    const spere_radius = 0.04 * Math.min(demWidth / geoWidth, demHeight / demWidth)
...
function regSHRivSphere() {
    var flag = 0
    let bt = $('.switch')[1]
    let circl = $('.switch_circle')[1]
    bt.addEventListener('click', () => {
        bt.classList.toggle('switch_active')
        circl.classList.toggle('circle_right')
        circl.classList.toggle('circle_left')
        const group = scene1.getObjectByName('riv_sphere_group');
        if (flag == 0) {
            if (group) {
                group.traverse(s => {
                    s.visible = false
...
function regSHCrossSphere() {
    var flag = 0
    let bt = $('.switch')[2]
    let circl = $('.switch_circle')[2]
    bt.addEventListener('click', () => {
        bt.classList.toggle('switch_active')
        circl.classList.toggle('circle_right')
        circl.classList.toggle('circle_left')
        const group = scene1.getObjectByName('cross_sphere_group');
        if (flag == 0) {
            if (group) {
                group.traverse(s => {
                    s.visible = false
...
function regSHRivPolyline() {
    var flag = 0
    let bt = $('.switch')[3]
    let circl = $('.switch_circle')[3]
    bt.addEventListener('click', () => {
        bt.classList.toggle('switch_active')
        circl.classList.toggle('circle_right')
        circl.classList.toggle('circle_left')
        const group = scene1.getObjectByName('riv_polyline_group');
        if (flag == 0) {
            if (group) {
                group.traverse(s => {
                    s.visible = false
...
function regSHMeshPolyline() {
    var flag = 0
    let bt = $('.switch')[4]
    let circl = $('.switch_circle')[4]
    bt.addEventListener('click', () => {
        bt.classList.toggle('switch_active')
        circl.classList.toggle('circle_right')
        circl.classList.toggle('circle_left')
        const group = scene1.getObjectByName('seg_group');
        if (flag == 0) {
            if (group) {
                group.traverse(s => {
                    s.visible = false
...
function regSHEleGrid(ovList) {
    var flag = 0
    let bt = $('.switch')[6]
    let circl = $('.switch_circle')[6]
    bt.addEventListener('click', () => {
        bt.classList.toggle('switch_active')
        circl.classList.toggle('circle_right')
        circl.classList.toggle('circle_left')
