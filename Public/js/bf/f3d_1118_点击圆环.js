...
function initClick(drawRect, tdtMap, scene, render, camera, demMesh, bboxGeo,
    lineDashMaterial, ringWaveMaterial, minEle, maxEle) {
    ...
    function ThreeClickHD(event, tdtMap, render, camera, mesh, fontSize) {
        ...
        if (intersects.length > 0) {
            ...
            if (!dem_marker) {
                ...
                dem_marker = new THREE.Line(label_line_geometry, lineDashMaterial)
                ...
                p_x_geo = (minX + (p_x * geo_width) / dem_width).toFixed(5)
                p_y_geo = (minY + (p_y * geo_height) / dem_height).toFixed(5)
                dem_click_info = createCoordSprite(p_x_geo, p_y_geo, ele, ep, '#ffffff', fontSize)
                ...
                //     side: THREE.DoubleSide,
                // })
                // dem_click_ring = new THREE.Mesh(ring_geometry, ring_meterial)
                ...
                //     // dot.lookAt(dot.position.clone().add(worldNormal));
                ...
                const mesh_width_seg = demMesh.geometry.parameters.widthSegments
                const ring_radius = 0.25 / mesh_width_seg
                const ring_geometry = new THREE.CircleGeometry(ring_radius, 32);
                dem_click_ring = new THREE.Mesh(ring_geometry, ringWaveMaterial)
                dem_click_ring.position.copy(point.clone().setY(point.y + 0.00001))
                dem_click_ring.lookAt(dem_click_ring.position.clone().add(worldNormal));
                dem_click_ring.rotateZ(-Math.PI / 2);
                threeClickGroup.add(dem_click_ring)
                ...
                p_x_geo = (minX + (p_x * geo_width) / dem_width).toFixed(5)
                p_y_geo = (minY + (p_y * geo_height) / dem_height).toFixed(5)
                updateCoordSprite(dem_click_info, ep, p_x_geo, p_y_geo, ele)
                dem_marker.geometry.setFromPoints([point, ep])
                ...
                //     dem_click_ring.localToWorld(vector)
                //     const dot = new THREE.Mesh(dot_geometry, dot_meterial)
                //     dot.position.copy(vector.clone().setY(vector.y + 0.0001))
                //     dots.push(dot)
                //     threeClickGroup.add(dot)
                // }
            ...
            if (!geo_marker) {
                ...
                geo_marker = new T.Marker(new T.LngLat(p_x_geo, p_y_geo), { icon: icon });
    ...
    function c_point_tdt_click_point_ins(point, bbox_dem, bboxGeo, meshDem) {
        ...
        if (intersects.length > 0) {
            ...
            if (!dem_marker) {
                ...
                const p_x = canvas_x + dem_width / 2
                const p_y = dem_height / 2 - canvas_z
                p_x_geo = (dragBoxMinX + (p_x * geo_width) / dem_width).toFixed(5)
                p_y_geo = (dragBoxMinY + (p_y * geo_height) / dem_height).toFixed(5)
                dem_click_info = createCoordSprite(p_x_geo, p_y_geo, ele, ep, '#ffffff', fontSize)
                threeClickGroup.add(dem_click_info)
...
async function c_polyline_riv(scene, meshDem, bboxDem, riverInsList, bboxGeo) {
    ...
    return [rivSphereGroup, crossSphereGroup, riverGroup]
}
