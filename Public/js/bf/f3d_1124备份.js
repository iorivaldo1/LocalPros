        ...
        if (intersects.length > 0) {
            ...
            reader.onload = function (e) {
                ...
                textureLoader.load(e.target.result, (texture) => {
                    createSpriteFromTexture(texture, ep, inBboxPointList[i][2], photoGroup);
                });
            };
...
function createSpriteFromTexture(texture, cross_point, photo_name, feature_group) {
    const material = new THREE.SpriteMaterial({
        map: texture,
        transparent: true,
        opacity: 0.9
...
function setupPhotoClickDetection(renderer, camera, scene) {
    ...
    function onMouseClick(event) {
        ...
        if (photoGroup) {
            ...
            if (photoSprite) {
                // console.log(photoSprite)
                // console.log('点击了照片:', photoSprite.object.userData);
    ...
    if (texture && texture.image) {
        ...
        if (newWindow) {
            newWindow.document.write(`
                <html>
                    <head><title>Sprite纹理</title></head>
                    <body style="margin:0; padding:20px; text-align:center; background:#f0f0f0;">
                        <h3>纹理图像预览</h3>
                        <img src="${texture.image.src || getImageDataUrl(texture.image)}" 
