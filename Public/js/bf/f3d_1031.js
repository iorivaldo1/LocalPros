...
async function c_mesh_dem(demData, camera, m_meshname) {
    ...
    for (let i = 0; i < elevationData.length; i++) {
        if (maxElevation == minElevation) {
            ...
            positions[i * 3 + 2] =
                (elevationData[i] - minElevation) / (maxElevation - minElevation) // 设置Z坐标
        }
    }
...
//画tdt中的dem网格线
async function drawGridAndEleLabel(
    DEMUrl,
    bounds,
    demOverLayerList,
    showLableFlag = 0
...
async function cal_riv_cross_mesh_grid(scene, meshDem, bboxDem, riverInsList, bboxGeo) {
    ...
    // const pa = wireframeGeometry.getAttribute('position')
    // const start = new THREE.Vector3()
    // start.fromBufferAttribute(pa, 0)
    // console.log('1', start)
    ...
    // start2.fromBufferAttribute(positionAttribute, 0)
    // console.log('2', start2)
    // meshDem.localToWorld(start2)
    // console.log('3', start2)
    ...
    for (let i = 0; i < riv_point_three.length - 1; i++) {
        const riv_sp = riv_point_three[i]
        const riv_ep = riv_point_three[i + 1]
        const riv_line = [riv_sp, riv_ep]
        // console.log(riv_line)
        for (let j = 0; j < positionAttribute.count - 1; j += 2) {
            const start = new THREE.Vector3()
            const end = new THREE.Vector3()
            start.fromBufferAttribute(positionAttribute, j)
            end.fromBufferAttribute(positionAttribute, j + 1)
            //*****
            ...
            if (ins_point1.intersects) {
                // console.log(riv_line, seg_line, ins_point1.intersectionPoint)
