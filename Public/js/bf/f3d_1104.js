...
async function c_point_bridge(scene, meshDem, bboxDem, bridgePointList, bboxGeo) {
    ...
    //标志点大小
    const spere_radius = 0.06 * Math.min(demWidth / geoWidth, demHeight / demWidth)
    ...
    rivList.forEach(river => {
        ...
        if (bboxGeo.intersects(river[0].getBounds())) {
            ...
            for (let i = 0; i < riv_coords.length - 1; i++) {
                //线中点
                const p_mid_s = [riv_coords[i].lng, riv_coords[i].lat]
                const p_mid_e = [riv_coords[i + 1].lng, riv_coords[i + 1].lat]
                const p_mid = new T.LngLat((p_mid_s[0] + p_mid_e[0]) / 2, (p_mid_s[1] + p_mid_e[1]) / 2)
                if (bboxGeo.contains(p_mid)) {
                    single_line.push(p_mid_s)
                    single_line.push(p_mid_e)
                }
            }
            ...
            if (p_index.length > 0) {
                ...
                if (single_line.length > 0) {
                    ...
                    const slice = [new T.LngLat(cross_point_list[0][0],cross_point_list[0][1]),new T.LngLat(cross_point_list[1][0],cross_point_list[1][1])]
                    riv_ins.push([slice, river[1]])
                }
