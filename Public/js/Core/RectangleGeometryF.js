...
import RectangleGeometryFLibrary from "./RectangleGeometryFLibrary.js";
...
function constructRectangle(RectangleGeometryF, computedOptions) {
  const vertexFormat = RectangleGeometryF._vertexFormat;
  const ellipsoid = RectangleGeometryF._ellipsoid;
  ...
  for (let row = rowStart; row < rowEnd; ++row) {
    for (let col = 0; col < width; ++col) {
      RectangleGeometryFLibrary.computePosition(
        computedOptions,
        ellipsoid,
        vertexFormat.st,
        row,
...
function constructExtrudedRectangle(RectangleGeometryF, computedOptions) {
  const shadowVolume = RectangleGeometryF._shadowVolume;
  const offsetAttributeValue = RectangleGeometryF._offsetAttribute;
  const vertexFormat = RectangleGeometryF._vertexFormat;
  const minHeight = RectangleGeometryF._extrudedHeight;
  const maxHeight = RectangleGeometryF._surfaceHeight;
  const ellipsoid = RectangleGeometryF._ellipsoid;
  ...
  if (shadowVolume) {
    ...
    RectangleGeometryF._vertexFormat = newVertexFormat;
  }

  const topBottomGeo = constructRectangle(RectangleGeometryF, computedOptions);

  if (shadowVolume) {
    RectangleGeometryF._vertexFormat = vertexFormat;
  }
...
function computeRectangle(rectangle, granularity, rotation, ellipsoid, result) {
  ...
  const computedOptions = RectangleGeometryFLibrary.computeOptions(
    rectangle,
    granularity,
    rotation,
    0,
 ...
 * const geometry = Cesium.RectangleGeometryF.createGeometry(rectangle);
 */
function RectangleGeometryF(options) {
  console.log("AAA")
  ...
  this._workerName = "createRectangleGeometryF";
 ...
 * @type {number}
 */
RectangleGeometryF.packedLength =
 ...
 *
 * @param {RectangleGeometryF} value The value to pack.
 * @param {number[]} array The array to pack into.
 * @param {number} [startingIndex=0] The index into the array at which to start packing the elements.
 *
 * @returns {number[]} The array that was packed into
 */
RectangleGeometryF.pack = function (value, array, startingIndex) {
  //>>includeStart('debug', pragmas.debug);
 ...
 * @param {RectangleGeometryF} [result] The object into which to store the result.
 * @returns {RectangleGeometryF} The modified result parameter or a new RectangleGeometryF instance if one was not provided.
 */
RectangleGeometryF.unpack = function (array, startingIndex, result) {
  //>>includeStart('debug', pragmas.debug);
  Check.defined("array", array);
  //>>includeEnd('debug');
  ...
  if (!defined(result)) {
    ...
    return new RectangleGeometryF(scratchOptions);
  }
 ...
 */
RectangleGeometryF.computeRectangle = function (options, result) {
  options = defaultValue(options, defaultValue.EMPTY_OBJECT);
 ...
 *
 * @param {RectangleGeometryF} RectangleGeometryF A description of the rectangle.
 * @returns {Geometry|undefined} The computed vertices and indices.
 *
 * @exception {DeveloperError} Rotated rectangle is invalid.
 */
RectangleGeometryF.createGeometry = function (RectangleGeometryF) {
  if (
    CesiumMath.equalsEpsilon(
      RectangleGeometryF._rectangle.north,
      RectangleGeometryF._rectangle.south,
      CesiumMath.EPSILON10,
    ) ||
    CesiumMath.equalsEpsilon(
      RectangleGeometryF._rectangle.east,
      RectangleGeometryF._rectangle.west,
      CesiumMath.EPSILON10,
    )
  ) {
    return undefined;
  ...
  let rectangle = RectangleGeometryF._rectangle;
  const ellipsoid = RectangleGeometryF._ellipsoid;
  const rotation = RectangleGeometryF._rotation;
  const stRotation = RectangleGeometryF._stRotation;
  const vertexFormat = RectangleGeometryF._vertexFormat;

  const computedOptions = RectangleGeometryFLibrary.computeOptions(
    rectangle,
    RectangleGeometryF._granularity,
    rotation,
    stRotation,
    rectangleScratch,
    nwScratch,
  ...
  const surfaceHeight = RectangleGeometryF._surfaceHeight;
  const extrudedHeight = RectangleGeometryF._extrudedHeight;
  ...
  computedOptions.lonScalar = 1.0 / RectangleGeometryF._rectangle.width;
  computedOptions.latScalar = 1.0 / RectangleGeometryF._rectangle.height;
  computedOptions.tangentRotationMatrix = tangentRotationMatrix;

  let geometry;
  let boundingSphere;
  rectangle = RectangleGeometryF._rectangle;
  if (extrude) {
    geometry = constructExtrudedRectangle(RectangleGeometryF, computedOptions);
    ...
    geometry = constructRectangle(RectangleGeometryF, computedOptions);
    ...
    if (defined(RectangleGeometryF._offsetAttribute)) {
      const length = geometry.attributes.position.values.length;
      const offsetValue =
        RectangleGeometryF._offsetAttribute === GeometryOffsetAttribute.NONE
          ? 0
          : 1;
  ...
  return new Geometry({
    ...
    offsetAttribute: RectangleGeometryF._offsetAttribute,
  });
};
 ...
 * @private
 */
RectangleGeometryF.createShadowVolume = function (
  RectangleGeometryF,
  minHeightFunc,
  maxHeightFunc,
) {
  const granularity = RectangleGeometryF._granularity;
  const ellipsoid = RectangleGeometryF._ellipsoid;

  const minHeight = minHeightFunc(granularity, ellipsoid);
  const maxHeight = maxHeightFunc(granularity, ellipsoid);

  return new RectangleGeometryF({
    rectangle: RectangleGeometryF._rectangle,
    rotation: RectangleGeometryF._rotation,
    ellipsoid: ellipsoid,
    stRotation: RectangleGeometryF._stRotation,
...
function textureCoordinateRotationPoints(RectangleGeometryF) {
  if (RectangleGeometryF._stRotation === 0.0) {
    return [0, 0, 0, 1, 1, 0];
  }

  const rectangle = Rectangle.clone(
    RectangleGeometryF._rectangle,
  ...
  const rotation = RectangleGeometryF._rotation - RectangleGeometryF._stRotation;
  ...
  const boundingRectangle = RectangleGeometryF.rectangle;
  const toDesiredInComputed = Matrix2.fromRotation(
    RectangleGeometryF._stRotation,
    rotation2DScratch,
  );
...
Object.defineProperties(RectangleGeometryF.prototype, {
  /**
   * @private
   */
  rectangle: {
...
export default RectangleGeometryF;
