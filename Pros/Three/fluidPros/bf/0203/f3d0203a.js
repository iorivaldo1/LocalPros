...
function regSHClickQuery(showFlag, domID, groupName, ovList) {
    ...
    ovList.forEach(ov => {
        if (ov && ov.options && ov.options.name == 'geo_marker_query') {
            ov.hide()
        }
    })
...
function regSHClickRing(showFlag, domID, groupName, ovList) {
    ...
    ovList.forEach(ov => {
        if (ov && ov.options && ov.options.name == 'geo_marker_ring') {
            ov.hide()
        }
    })
...
//注册污染模拟开关
function regSHFluidSim(showFlag, domID, fluidSim) {
    $('#' + domID).off('click')
    $('#' + domID).on('click', HD1)
    ...
    if (fluidSim) {
        fluidSim.setEnabled(!!showFlag[domID])
    }

    function HD1() {
        bt.classList.toggle('switch_active')
        circle.classList.toggle('circle_right')
        showFlag[domID] = !showFlag[domID]
        if (fluidSim) {
            fluidSim.setEnabled(!!showFlag[domID])
        }
    }
...
//初始化流体污染模拟
async function initFluidSimulation(options) {
    const {
        renderer,
        scene,
        camera,
    ...
    if (!window.FluidShaders) {
        console.warn('FluidShaders 未加载，跳过流体模拟初始化')
        return null
    }

    const TERRAIN_SIZE = terrainMesh.geometry.parameters.width
    const TERRAIN_ASPECT = terrainMesh.geometry.parameters.height / terrainMesh.geometry.parameters.width
    const HEIGHT_SCALE = 1.0

    // 根据 terrainMesh 的实际尺寸动态计算物理参数
    // terrain8.tif 的参考尺寸为 512x512，对应的 mesh width 约为 10
    const REFERENCE_MESH_SIZE = terrainMesh.geometry.parameters.width // terrain8.tif 生成的 mesh 宽度
    const sizeFactor = TERRAIN_SIZE / REFERENCE_MESH_SIZE
    
    // 基础参考参数（针对 10x10 大小的mesh优化）
    const BASE_GRAVITY = 15.0
    const BASE_PIPE_AREA = 0.20  // 降低从0.35到0.20，减少流速
    const BASE_PIPE_LENGTH = 1.2  // 增大从0.8到1.2，减缓流速
    const BASE_DT = 0.003  // 降低从0.005到0.003，减缓速度
    const BASE_INJECT_AMOUNT = 2.6
    const BASE_INJECT_RADIUS = 0.01
    
    const params = {
        gravity: BASE_GRAVITY,  // 重力不随尺寸变化
        pipeArea: BASE_PIPE_AREA * sizeFactor,  // 管道面积按尺寸缩放
        pipeLength: BASE_PIPE_LENGTH * sizeFactor,  // 管道长度按尺寸缩放
        dt: BASE_DT / sizeFactor,  // 时间步长反比缩放（更大的地形需要更小的时间步）
        injectAmount: BASE_INJECT_AMOUNT * sizeFactor * sizeFactor,  // 注入量按面积缩放
        injectRadius: BASE_INJECT_RADIUS * sizeFactor,  // 注入半径按尺寸缩放
        evaporation: 0.0000
    }

    const tiff = await GeoTIFF.fromArrayBuffer(demArrayBuffer)
    const image = await tiff.getImage()
    const rasters = await image.readRasters()
    ...
    const simHeightData = new Float32Array(heightDataRGBA)
    ...
    if (terrainMesh && terrainMesh.material && terrainMesh.material.map) {
        domTexture = terrainMesh.material.map
        domTexture.needsUpdate = true
    } else if (domCanvas) {
        domTexture = new THREE.CanvasTexture(domCanvas)
    ...
    // 增大流体模拟的分辨率倍数，提高采样精度
    const FLUID_RESOLUTION_SCALE = 1.0  // 分辨率放大2倍
    const fluidSimWidth = Math.floor(tiffWidth * FLUID_RESOLUTION_SCALE)
    const fluidSimHeight = Math.floor(tiffHeight * FLUID_RESOLUTION_SCALE)
    
    console.log('=== 流体模拟分辨率提升 ===')
    console.log('原始 DEM 分辨率:', tiffWidth, 'x', tiffHeight, '=', (tiffWidth * tiffHeight).toLocaleString(), '像素')
    console.log('流体模拟分辨率:', fluidSimWidth, 'x', fluidSimHeight, '=', (fluidSimWidth * fluidSimHeight).toLocaleString(), '像素')
    console.log('分辨率提升倍数: ' + FLUID_RESOLUTION_SCALE + 'x')
    console.log('总采样点增加:', ((fluidSimWidth * fluidSimHeight) / (tiffWidth * tiffHeight)).toFixed(1) + 'x')

    function createRenderTarget(useLinear = false) {
        return new THREE.WebGLRenderTarget(fluidSimWidth, fluidSimHeight, {
    ...
    const fluxUniforms = {
        ...
        uFluxTex: { value: null },
        uResolution: { value: new THREE.Vector2(fluidSimWidth, fluidSimHeight) },
        uDt: { value: params.dt },
    ...
    const pollutionUniforms = {
        ...
        uDecayRate: { value: 0.0002 }  // 增大衰减率，从0.00001增加到0.0002，防止污染无限扩张
    }
    ...
    const terrainMat = new THREE.ShaderMaterial({
        uniforms: {
            ...
            uHasTerrainTex: { value: domTexture ? 1.0 : 0.0 },
            uTerrainColor: { value: new THREE.Color(0x3a5f3a) },
            uPollutionColor: { value: new THREE.Color(0xcc6633) },
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                if (uHasTerrainTex > 0.5) {
                    baseColor = texture2D(uTerrainTex, vUv).rgb;
                }
                float pollutionAmount = texture2D(uPollutionTex, flippedUV).r;
                // 降低阈值，让污染面更容易显示
                float pollutionStrength = smoothstep(0.0005, 0.08, pollutionAmount);
                vec3 finalColor = mix(baseColor, uPollutionColor, pollutionStrength * 0.85);
    ...
    // 水体mesh的分段数需要是纹理分辨率的2倍，以充分展示纹理细节
    const waterSegmentsWidth = Math.floor(fluidSimWidth ) - 1
    const waterSegmentsHeight = Math.floor(fluidSimHeight ) - 1
    
    const waterGeo = new THREE.PlaneGeometry(
        TERRAIN_SIZE,
        TERRAIN_SIZE * TERRAIN_ASPECT,
        waterSegmentsWidth,
        waterSegmentsHeight
    )
    console.log('=== 水体 Mesh 尺寸信息 ===')
    console.log('Water Mesh Width:', TERRAIN_SIZE)
    console.log('Water Mesh Height:', TERRAIN_SIZE * TERRAIN_ASPECT)
    console.log('Water Mesh WidthSegments:', waterSegmentsWidth)
    console.log('Water Mesh HeightSegments:', waterSegmentsHeight)
    console.log('流体纹理分辨率:', fluidSimWidth, 'x', fluidSimHeight)
    console.log('Mesh分段数/纹理分辨率比:', (waterSegmentsWidth / fluidSimWidth).toFixed(2) + 'x')
    ...
    waterRenderMat.uniforms.uResolution.value.set(fluidSimWidth, fluidSimHeight)
    ...
    waterRenderMat.uniforms.uWaterTex.value = rtWater[0].texture

    const waterSurface = new THREE.Mesh(waterGeo, waterRenderMat)
    waterSurface.rotation.x = -Math.PI / 2
    waterSurface.position.y = 0.01
    waterSurface.name = 'water_surface'

    const raycaster = new THREE.Raycaster()
    const mouse = new THREE.Vector2()
    const injectUV = new THREE.Vector2()
    ...
    function setEnabled(flag) {
        if (flag && !enabled) {
            enabled = true
            prevMaterial = terrainMesh.material
            terrainMesh.material = terrainMat
            if (!waterSurface.parent) {
            ...
            if (waterSurface.parent) {
                waterSurface.parent.remove(waterSurface)
            }
            if (prevMaterial) {
                terrainMesh.material = prevMaterial
            }
        }
    }
    ...
    function dispose() {
        ...
        rtWater.forEach(rt => rt.dispose())
        rtFlux.forEach(rt => rt.dispose())
        rtPollution.forEach(rt => rt.dispose())
        heightTexture.dispose()
        simHeightTexture.dispose()
        if (domTexture && domCanvas) {
            domTexture.dispose()
        }
        terrainMat.dispose()
    ...
    function updateParams(newParams) {
        if (newParams.gravity !== undefined) {
            params.gravity = newParams.gravity
            fluxUniforms.uGravity.value = newParams.gravity
        }
        if (newParams.pipeArea !== undefined) {
            params.pipeArea = newParams.pipeArea
            fluxUniforms.uPipeArea.value = newParams.pipeArea
        }
        if (newParams.pipeLength !== undefined) {
            params.pipeLength = newParams.pipeLength
            fluxUniforms.uPipeLength.value = newParams.pipeLength
            waterUniforms.uPipeLength.value = newParams.pipeLength
        }
        if (newParams.dt !== undefined) {
            params.dt = newParams.dt
            fluxUniforms.uDt.value = newParams.dt
            waterUniforms.uDt.value = newParams.dt
        }
        if (newParams.injectAmount !== undefined) {
            params.injectAmount = newParams.injectAmount
            waterUniforms.uInjectAmount.value = newParams.injectAmount
        }
        if (newParams.injectRadius !== undefined) {
            params.injectRadius = newParams.injectRadius
            waterUniforms.uInjectRadius.value = newParams.injectRadius
        }
        if (newParams.evaporation !== undefined) {
            params.evaporation = newParams.evaporation
            waterUniforms.uEvaporation.value = 1.0 - newParams.evaporation
        }
    }
